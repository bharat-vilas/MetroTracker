...
const decodeBase64 = (encodedData: string) => {
  try {
    ...
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(cleanedData)) {
      console.warn(
        "Data does not appear to be valid base64:",
...
export const apiService = {
        ...
        const result = {
          ...
          routes: data.result.map((route: any) => ({
            ...
            startTime: route.start_time
              ? Math.floor(route.start_time / 60)
                  .toString()
                  .padStart(2, "0") +
                ":" +
                (route.start_time % 60).toString().padStart(2, "0")
              : "",
            endTime: route.end_time
              ? Math.floor(route.end_time / 60)
                  .toString()
                  .padStart(2, "0") +
                ":" +
                (route.end_time % 60).toString().padStart(2, "0")
              : "",
          })),
        };
  ...
  async getStopsForRoute(routeName: string) {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/GetStopsData?data=${encodedRouteName}`
      );
      const data = await response.json();
  ...
  async getDisconnectedSegments() {
    try {
      const response = await fetch(
        `${BASE_URL}/GetDisconnectedSegments?data=ALL`
      );
      const encodedData = await response.text();
      return decodeBase64(encodedData);
    } catch (error) {
  ...
  async getAvlData() {
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        const encodedData = await response.text();
        console.log(
          "AVL Data raw response (first 200 chars):",
  ...
  async getAllStopsWithETAForRoutes(routeNames: string[]) {
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/GetStopsData?data=${encodedRouteNames}`
      );
      const data = await response.json();
                ...
                features: [
                  {
                    geometry: {
                      type: "LineString",
                      coordinates:
                        this.extractCoordinatesFromPolylineData(item),
                    },
                    properties: {
  ...
  extractCoordinatesFromPolylineData(item: any): number[][] {
    try {
      ...
      if (item.polyline_latlng?.features?.[0]?.geometry?.coordinates) {
        ...
        console.log(
          "Found coordinates in polyline_latlng.features[0].geometry.coordinates:",
          coords.length
        );
      ...
      if (item.coordinates && Array.isArray(item.coordinates)) {
        console.log(
          "Found coordinates in direct coordinates array:",
      ...
      if (item.polyline && typeof item.polyline === "string") {
        console.log(
          "Found encoded polyline string, length:",
          item.polyline.length
        );
